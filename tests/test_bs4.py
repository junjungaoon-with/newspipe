...
def extract_simple_info_from_html(html: str) -> dict:
    ...
    for article_outer_element in soup.find_all("div", id="contentsWrap"):
        ...
        try:
            num_comments = article_outer_element.find(
                "span", class_="sc-1n9vtw0-3"
            ).get_text()
        except:
            num_comments = 0
        ...
        # コメントのリストを作成
        for article_element in soup.find(
            "p", class_="sc-54nboa-0 deLyrJ yjSlinkDirectlink highLightSearchTarget"
        ):
...
html = fetch_html(
    "https://news.yahoo.co.jp/articles/970d355ffd6a3502c8ddb9ec15534fa0993d245a"
)
